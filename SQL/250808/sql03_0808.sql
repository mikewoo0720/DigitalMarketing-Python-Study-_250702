...
# 현재 내가 사용하고 있는 MySQL 워크벤치 안에 있는 전체 테이블 정보 값을 가지고 있는 시스템 테이블 = 메타 테이블
...
SELECT * FROM customers;
EXPLAIN SELECT * FROM customers;#찾아오는데 걸리는 시간등의 정보값을 보여줌 (ALL 394 전부 돌고 왔엉)

EXPLAIN SELECT * FROM customers WHERE id = 300; # const 1 //Primary Key
EXPLAIN SELECT * FROM customers WHERE email = 'user95976@test.com'; # ref =>참조
